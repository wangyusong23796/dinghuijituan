...
<body>
	...
	<div class="main-title">
		<h2><?php echo ($title); ?>内容列表</h2>
	</div>

	<div class="cf">
		<div class="fl">
		<?php if(($model["extend"]) == "1"): ?><div class="tools">
				<a class="btn" href="<?php echo U('edit',array('name'=>$name));?>">新 增</a>
				<button class="btn ajax-post confirm" target-form="ids" url="<?php echo U('del?name='.$name);?>">删 除</button>
			</div><?php endif; ?>
		</div>
		<!-- 高级搜索 -->
		<div class="search-form fr cf">
			<div class="sleft">
				...
				<a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('adminList','name='.$name,false);?>"><i class="btn-search"></i></a>
			</div>
		</div>
	</div>
	...
	<div class="data-table table-striped">
		<?php if(empty($custom_adminlist)): ?><!-- 数据列表 -->
	        <table>
				<thead>
					<tr>
						<th width="50">
				...
				<tbody>
					<?php if(is_array($_list)): $vo = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$lv): $mod = ($vo % 2 );++$vo;?><tr>
						<td><input class="ids" type="checkbox" value="<?php echo ($lv["id"]); ?>" name="ids[]"><?php echo ($lv["id"]); ?></td>
